...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($name) || empty($description) || empty($price) || empty($quantity) || empty($barcode)) {
        ...
        if ($id) {
            ...
            $sql = "UPDATE products SET name=?, description=?, price=?, quantity=?, barcode=?, updated_at=NOW() WHERE id=?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$name, $description, $price, $quantity, $barcode, $id]);
            $message = 'Product updated successfully!';
        } else {
            // Insert new product
            $sql = "INSERT INTO products (name, description, price, quantity, barcode, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())";
            $stmt = $pdo->prepare($sql);
